<template>
  <div class="main">
    <div class="title">
      <img :src="logo" alt="" />
      <div>即刻点名跑接力成绩单</div>
    </div>
    <div class="list-box">
      <div class="list" v-for="item in list" :key="item.id">
        <div class="number">{{ item.id }}</div>
        <div class="user">
          <img :src="item.head" class="head-url" alt="" />
          <img :src="item.level" class="level" v-if="item.level" alt="" />
          <div>
            <p :class="{ good: item.good == 1 }">{{ item.name }}</p>
            <p class="jk-num">
              <span v-if="item.alias">{{ item.alias }}</span
              >{{ item.number }}
            </p>
          </div>
        </div>
        <div class="km">{{ item.km }}</div>
      </div>
    </div>
  </div>
</template>

<script>
import level1 from "./img/1.png";
import level2 from "./img/2.png";
import level3 from "./img/3.png";

export default {
  data() {
    return {
      logo: "data:image/png;base64,",
      list: [
        {
          id: 1,
          good: 1,
          number: "JK0001",
          name: "迷思特玛",
          head: "https://cdn.jellow.site/FrNoNa-sk788pA-xgH9qrpN6quWh.jpg?imageMogr2/auto-orient/heic-exif/1/format/jpeg/thumbnail/300x300%3E",
          km: "227KM",
          alias: "状元",
          level: level1,
        },
        {
          id: 2,
          good: 1,
          number: "JK0002",
          name: "爱老公比永远多一天",
          head: "https://cdn.jellow.site/FtL6tp7h5AMYRfXOBNKAP5y1aEKy.jpg?imageMogr2/auto-orient/heic-exif/1/format/jpeg/thumbnail/300x300%3E",
          km: "174KM",
          alias: "榜眼",
          level: level2,
        },
        {
          id: 3,
          good: 1,
          number: "JK0003",
          name: "知道分子的时代",
          head: "https://cdn.jellow.site/FpRJq5d3pAXGT4S79TrsN8pMlSD4.png?imageMogr2/auto-orient/heic-exif/1/format/jpeg/thumbnail/300x300%3E",
          km: "149KM",
          alias: "探花",
          level: level3,
        },
        {
          id: 4,
          number: "JK0004",
          name: "礼峰",
          head: "https://cdn.jellow.site/FhcghLP6eBpSwoP_D3uyPLmEaxCk?imageMogr2/auto-orient/heic-exif/1/format/jpeg/thumbnail/300x300%3E",
          km: "145KM",
        },
        {
          id: 5,
          number: "JK0005",
          name: "方十三",
          head: "https://cdn.jellow.site/FvAW8vIeTAtIDg3L3y_WZSYKevk6.jpg?imageMogr2/auto-orient/heic-exif/1/format/jpeg/thumbnail/300x300%3E",
          km: "123KM",
        },
        {
          id: 6,
          number: "JK0006",
          name: "柏木",
          head: "https://cdn.jellow.site/Fuv3XeFJ9YKJwZhhHTf2micQLm5h.jpg?imageMogr2/auto-orient/heic-exif/1/format/jpeg/thumbnail/120x120%3E",
          km: "113KM",
        },
        {
          id: 7,
          number: "JK0007",
          name: "有谁不喜欢佐伊呢",
          head: "https://cdn.jellow.site/FtD45qy4BHrreCZe7zpAsmlvusgN.jpg?imageMogr2/auto-orient/heic-exif/1/format/jpeg/thumbnail/300x300%3E",
          km: "98KM",
        },
        {
          id: 8,
          number: "JK0008",
          name: "VG丶Evontion",
          head: "https://cdn.jellow.site/FlECeEu1ZybpHKbUZUASfqbh8oj2.jpeg?imageMogr2/auto-orient/heic-exif/1/format/jpeg/thumbnail/120x120%3E",
          km: "87KM",
        },
        {
          id: 9,
          number: "JK0009",
          name: "轩轩_kino",
          head: "https://cdn.jellow.site/FkRLRN5-RFuRi8ERAXwSp-O88iff.png?imageMogr2/auto-orient/heic-exif/1/format/jpeg/thumbnail/120x120%3E",
          km: "86KM",
        },
        {
          id: 10,
          number: "JK0010",
          name: "李发发发",
          head: "https://cdn.jellow.site/Fqg3XuOcigRqJLaiE3O2JMSUUGmV.jpg?imageMogr2/auto-orient/heic-exif/1/format/jpeg/thumbnail/300x300%3E",
          km: "85KM",
        },
      ],
    };
  },
  methods: {},
};
</script>
<style scoped lang="less">
.main {
  background-color: rgb(255, 228, 17);
  // background-color: #000;
  width: 100vw;
  min-height: 100vh;
  overflow: hidden;
  padding: 0.5rem 0.2rem;
  box-sizing: border-box;
  color: #fff;
  .title {
    text-align: center;
    font-size: 0.4rem;
    font-weight: bold;
    display: flex;
    align-items: center;
    justify-content: center;
    img {
      width: 0.5rem;
      height: 0.5rem;
      margin-right: 0.1rem;
    }
  }
  .list-box {
    margin: 0.5rem 0;
    background-color: rgba(255, 255, 255, 0.5);
    border-radius: 0.16rem;
    .list {
      display: flex;
      align-items: center;
      padding: 0.35rem 0.3rem;
      color: #666;
      .number {
        width: 0.5rem;
        text-align: left;
        color: #888;
        font-size: 0.28rem;
      }
      .user {
        display: flex;
        align-items: center;
        position: relative;
        .head-url {
          display: block;
          width: 1rem;
          height: 1rem;
          margin-right: 0.2rem;
          border-radius: 50%;
        }
        .level {
          display: block;
          position: absolute;
          top: -0.15rem;
          left: -0.15rem;
          width: 0.5rem;
          z-index: 10;
        }
        .good {
          font-weight: bold;
        }
        div {
          min-width: 0;
          flex: 1;
          .jk-num {
            margin-top: 0.1rem;
            font-size: 0.2rem;
            color: rgb(190, 190, 190);
            span {
              margin-right: 0.1rem;
              color: rgb(238, 76, 76);
            }
          }
        }
      }
      .km {
        margin-left: auto;
        font-size: 0.28rem;
      }
    }
  }
}
</style>